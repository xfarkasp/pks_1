...
void PcapParser::parseFrame(std::string path) {
    ...
    _protocolMap = setProtocolMap("Protocols\\protocols.txt", true);  //call method to map protocols from file
    ...
    _l4ProtocolMaps = setProtocolMap("Protocols\\l4.txt", true);
    ...
    if (pcap_t* pcap = pcap_open_offline(path.c_str(), errBuff)) //read file content to pcap_t pointer
    {
        ...
        std::cout << "can not open pcap file, type command again with new file!" << std::endl;
        exit(0);
    }
}
...
std::vector<std::string> PcapParser::getFrameType(int typeSize, std::vector<unsigned int> data, bool ISL) {
    ...
    if (typeSize >= PcapParser::ETHERNET_II_MIN) {
        ...
        if (_protocolMap.find(typeSize) != _protocolMap.end())
...
void PcapParser::serializeYaml() {
    ...
    for (auto packet : _frames) {
        ...
        if (frameTypes.size() > 1 && frameTypes.at(1) == "IPv4") {
            ...
            output << YAML::Key << "protocol" << YAML::Value << _l4ProtocolMaps[stoi(hex_string, 0, 16)];

            if (_l4ProtocolMaps[stoi(hex_string, 0, 16)] == "UDP" || _l4ProtocolMaps[stoi(hex_string, 0, 16)] == "TCP") {
            ...
            else if (_l4ProtocolMaps[stoi(hex_string, 0, 16)] == "ICMP") {
